...
export class Artist {
  ...
  rectangle(xOrRect, y, width, height) {
    let rect;
    if (typeof xOrRect === "object") {
      rect = xOrRect;
    } else {
      rect = { x: xOrRect, y, width, height };
    }
    this.definitions.push(buildDStringForRectangle(rect.x, rect.y, rect.width, rect.height));
    return this;
  }

  twoPointRectangle(point1, point2) {
    let point1o = PointMath.toXYObject(point1);
    let point2o = PointMath.toXYObject(point2)

    const width = Math.abs(point2o.x - point1o.x);
    const height = Math.abs(point2o.y - point1o.y);
    const tlcX = Math.min(point1o.x, point2o.x);
    const tlcY = Math.min(point1o.y, point2o.y);
    const tlc = [tlcX, tlcY];
    return this.rectangle(tlc[0],tlc[1], width, height);
  }
...
${this.elements
  ...
  adjustViewboxToFitContent(paddingX=0, paddingY=0) {
    const boundingBox = this.getContentBoundingBox();
    if (boundingBox) {
      boundingBox.x -= paddingX / 2
      boundingBox.width += paddingX
      boundingBox.y -= paddingY / 2
      boundingBox.height += paddingY
