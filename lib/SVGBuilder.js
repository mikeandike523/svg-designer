 ...
 * @returns {string}
 */
export function compilePathElement(d, stroke, strokeWidth=1.0, fill, opacity = 1.0, transform) {
  let transformString = "";
  if (transform) {
...
export default class SVGBuilder {
   ...
   * @param {GaussianBlur} gaussianBlur
   */
  addPath(d, stroke, strokeWidth, fill, opacity, transform, dropShadow, gaussianBlur) {
  ...
  artist(stroke, strokeWidth, fill, opacity, transform, dropShadow, gaussianBlur) {
    return new Artist(
      stroke,
      strokeWidth,
      fill,
